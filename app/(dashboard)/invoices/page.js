...
export default function InvoicesPage() {
  ...
  async function loadDocs() {
    ...
    if (!user) return
    const { data } = await supabase.from('quotes').select('*').eq('user_id', user.id).eq('document_type', 'invoice').order('created_at', { ascending: false })
  ...
  return (
    <div className="space-y-5">
      <div><h1 className="text-2xl font-bold text-[var(--text-primary)]" style={{ fontFamily: 'Instrument Serif, serif' }}>Invoices</h1><p className="text-sm text-[var(--text-muted)]">{docs.length} invoices</p></div>
      ...
      <div className="relative">
        ...
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search invoices..." className="w-full pl-10 pr-4 py-3 rounded-xl bg-[var(--surface-1)] border border-[var(--border)] text-[var(--text-primary)] placeholder:text-[var(--text-muted)] text-sm" style={{ minHeight: '44px' }} />
      </div>

      {filtered.length === 0 ? (
        <div className="bg-[var(--surface-1)] rounded-2xl border border-[var(--border)] p-12 text-center">
          <svg className="w-12 h-12 mx-auto mb-3 text-[var(--text-muted)]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.2} d="M9 14l6-6m-5.5.5h.01m4.99 5h.01M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16l3.5-2 3.5 2 3.5-2 3.5 2z" /></svg>
          <h3 className="text-lg font-semibold text-[var(--text-primary)] mb-1">No invoices yet</h3>
          <p className="text-sm text-[var(--text-muted)]">Invoices are generated from delivered goods or services.</p>
        </div>
      ) : (
